...
@implementation SongRKObjectManager {
...
- (void)getKilograppObjectsAtPath:(NSString *)path
    ...
    [objectManager getObjectsAtPath:[NSString stringWithFormat:@"%@", @"/songs/api/songs/"]
                         parameters:params
                            success:success
                            failure:failure];
}
...
+ (SongRKObjectManager *)manager {
    static SongRKObjectManager *manager = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        manager = [[SongRKObjectManager alloc] init];
    });
    return manager;
}
